...
and appends a "page" of DOM as a child of parentNode. This is sometimes used to append elements to a list (like block children) and sometimes used to fill in a single-capacity slot (like render.blockBody). It is up to the caller to remove the previous child of parentNode if needed.
 */
const render = {
  page: (store, parentNode, uid, withBackrefs = true) => {
    ...
    if (withBackrefs && refs) {
      ...
      render.backrefs(store, backrefsListElement.children[1], refs)
    }

    parentNode.appendChild(element)
    return element
  },
  backrefs: (store, parent, refs) => {
    sortByLastEdited(store, refs)
    for (let backref of refs) {
      const backrefFrame = templates.backrefFrame.cloneNode(true)
      render.breadcrumb(store, backrefFrame.children[0], backref)
      render.block(store, backrefFrame.children[1], backref)
      parent.appendChild(backrefFrame)
    }
  },
  block: (store, parentNode, uid, idx, collapsed) => {
    ...
    render.blockBody(store, body, string)
    ...
    if (collapsed || (store.collapsed && store.collapsed[uid])) {
      ...
      for (let kid of kids || []) {
        render.block(store, childrenContainer, kid)
      }
    }
  ...
  blockFramed: (theStore, parentNode, uid) => {
    const blockFocusFrame = templates.blockFocusFrame.cloneNode(true)
    render.breadcrumb(theStore, blockFocusFrame.children[0], uid)
    render.block(theStore, blockFocusFrame.children[1], uid)
    ...
    if (backRefs) {
      ...
      for (let backref of backRefs) {
        render.block(theStore, backrefsListElement.children[1], backref)
      }
    }
    parentNode.appendChild(blockFocusFrame)
  },
  breadcrumb: (store, parent, blockId) => {
    ...
    if (list.length > 0) {
      ...
      render.blockBody(store, node, title)
      node.dataset.title = title
      parent.appendChild(node)
      for (let i = list.length - 2; i >= 0; i--) {
        const node = templates.breadcrumbBlock.cloneNode(true)
        const nodeBody = node.children[1]
        render.blockBody(store, nodeBody, truncateElipsis(list[i].string, 22))
  ...
  blockBody: (store, parent, text, editMode = false) => {
    ...
    const transformComputeElement = (store, el, editMode,) => {
      ...
      switch (pageTitle) {
        ...
        case "query":
          ...
          render.backrefs(store, queryFrame, result)
    ...
    const stack = [parent]
    const matches = text.matchAll(render._parseRegex)
    ...
    for (let match of matches) {
      ...
      if (match[1] !== undefined) {
        ...
        if (stackTop.className === "page-ref__body") {
          ...
          // this replaces titleHere = stackTop.innerText
          const titleHere = text.substring(stackTop.parentNode.startIdx + 2, idx)
          stackTop.parentNode.dataset.title = titleHere
  ...
  resultSet: (parent, resultArray, resultFrameElement, startIdx = 0) => {
