...
Live (probably) at traversetext.com
...
Clone on an Ubuntu machine, then run `.sh/setup-dev.sh`. This will create empty data folders, install nodejs, and start the site and API server. If all goes well the site will be up on `localhost:8081`. Whenever you change the code, you can reload the page to see the changes. Otherwise it's up to you to read and debug the script.
